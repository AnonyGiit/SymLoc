    ...
    if (ExternalCalls == ExternalCallPolicy::All) { // don't bother checking uniqueness
      *ai = optimizer.optimizeExpr(*ai, true);
      ...
      wordIndex += (ce->getWidth()+63)/64;
        ...
        ref<Expr> arg = toUnique(state, *ai);
      // *THX start
      // *THX add for replace the symbolic arguments
...
void Executor::executeAllocForMalloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
   ...
   std::map< ExecutionState*, std::vector<SeedInfo> >::iterator it =
    ...
    if (it!=seedMap.end()) { // In seed mode we need to add this as a
      ...
      for (std::vector<SeedInfo>::iterator siit = it->second.begin(),
             siie = it->second.end(); siit != siie; ++siit) {
        SeedInfo &si = *siit;
        KTestObject *obj = si.getNextInput(mo_buffer, NamedSeedMatching);

        if (!obj) {
          if (ZeroSeedExtension) {
            std::vector<unsigned char> &values = si.assignment.bindings[array];
            values = std::vector<unsigned char>(mo_buffer->size, '\0');
          ...
          if (obj->numBytes != mo_buffer->size &&
              ((!(AllowSeedExtension || ZeroSeedExtension)
                && obj->numBytes < mo_buffer->size) ||
               (!AllowSeedTruncation && obj->numBytes > mo_buffer->size))) {
	    std::stringstream msg;
	    msg << "replace size mismatch: "
		<< mo_buffer->name << "[" << mo_buffer->size << "]"
		...
		<< " in test\n";
            ...
            values.insert(values.begin(), obj->bytes,
                          obj->bytes + std::min(obj->numBytes, mo_buffer->size));
            if (ZeroSeedExtension) {
              for (unsigned i=obj->numBytes; i<mo_buffer->size; ++i)
                values.push_back('\0');
            }
          }
        }
    ...
    //ObjectState *os = bindObjectInState(state, mo_buffer, false);
    ObjectState *os = os_buffer;
    ...
    //} else {
      ...
      //} else {
        for (unsigned i=0; i<mo_buffer->size; i++)
    ...
    */
    ...
    location.push_back(target->info->file);
    location.push_back(std::to_string(target->info->line));
    state.mallocLocation.insert(std::pair<std::string, std::vector<std::string>>(sym_name, location));
    location.clear();
    // get a symbolic Expr
  ...
  if (!isa<ConstantExpr>(address)){ //a symbolic variable?
    ...
    */
    ...
    if (isSymbolicAddress && hasSymbolicMallocVariable(address, mmm)){ // handle symbolic address and aviod normal symbolic variable by comand
        ...
        const MemoryObject *mo = shared_map[sym_name].first;
        //mo->name=sym_name;
        //TODO add new symbolic address to symbolics// done in executeAllocForMalloc
        ...
        for (int i = 0; i < pp->getNumKids(); i++){ //TODO need to find a better solution
