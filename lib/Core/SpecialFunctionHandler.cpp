...
void SpecialFunctionHandler::handleMalloc(ExecutionState &state,
  ...
  // *THX start
  // *THX* here we decide to make a malloc to a symbol
  ...
  std::mt19937 gen(rd());
  // select the symbolization mode here
...
void SpecialFunctionHandler::handleCheckMemoryAccess(ExecutionState &state,
  ...
  // *THX start
    ...
    if (isSymbolicAddress){ // handle symbolic address
        std::map<std::string, ObjectPair> shared_map = state.addressSpace.mobjects;
        const MemoryObject *mo = shared_map[sym_name].first;
        //TODO do not check memory access for symbolic address, as it's no need
        //address = ConstantExpr::create(mo->address, 64);
    }
    else { //handle a normal symbolic variable, should be an error
        klee_warning("memory error: check from a symbolic address");
    }*/
  }
  //*THX end
}
